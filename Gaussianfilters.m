% M. Varma, A. Zisserman. A statistical approach to material classification using image patch
% exemplars. IEEE Trans. Pattern Analysis and Machine Intelligence, 2009, 31(11):2032-2047.
%
function F=Gaussianfilters
...
% Fourier transform.
  ...
  NORIENT=1;              % Number of orientations
...
%       F(:,:,count+NEDGE)=makefilter(SCALEX(scale),0,2,rotpts,SUP);
  ...
  F(:,:,count)=normalise(fspecial('log',SUP,1));
  F(:,:,count+1)=normalise(fspecial('log',SUP,2));
  F(:,:,count+2)=normalise(fspecial('log',SUP,4));
...
function f=makefilter(scale,phasex,phasey,pts,sup)
  gx=gauss1d(1*scale,0,pts(1,:),phasex);%£¡£¡£¡£¡£¡£¡£¡£¡£¡£¡£¡£¡£¡£¡£¡£¡£¡
